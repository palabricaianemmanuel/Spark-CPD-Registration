...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!winners || !Array.isArray(winners) || winners.length === 0) {
      return res.status(400).json({ error: 'Missing or empty winners array' });
    }
    ...
    for (const winner of winners) {
      try {
        ...
        if (error) {
          results.push({ id: winner.id, success: false, error: error.message });
        } else {
          results.push({ id: winner.id, success: true });
        }
      } catch (err: any) {
        results.push({ id: winner.id, success: false, error: err.message });
      }
    }
    ...
    return res.status(200).json({
      success: true,
      summary: { total: winners.length, sent: successCount, failed: failCount },
      results,
    });
  } catch (err: any) {
